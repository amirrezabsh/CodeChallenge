...
class PlaceOrderView(APIView):
    ...
    def buy_from_exchange(self, amount, currency):
        ...
        print(
            f"Bought {amount} {currency} with {amount * currencies_price.get(currency)}$ value from exchange.")

    def checkout(self, currency, price):

        checkout_threshold = 10
        # Filtering not checkedout orders
        total_not_checkedout_orders = Order.objects.filter(
            currency=currency, is_checkedout=False)
        total_amount = total_not_checkedout_orders.aggregate(Sum('amount'))[
            'amount__sum']
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            order = Order.objects.create(
                customer=user, currency=currency, amount=amount)
